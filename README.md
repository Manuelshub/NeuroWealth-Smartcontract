...
The repository now contains the production workspaces rather than planned placeholders:

| Workspace | Directory | Purpose | Local setup |
|-----------|-----------|---------|-------------|
| Soroban vault contract | `neurowealth-vault/` | Core on-chain USDC vault, share accounting, strategy routing, and withdrawal queue logic | See the contract build/test commands below and `docs/MAINNET_CHECKLIST.md` |
| AI agent backend | `agent/` | Off-chain strategy runner and event-driven automation service | See `agent/README.md` |
| Next.js web frontend | `frontend/` | Canonical wallet/dashboard UI | See `frontend/README.md` |
| WhatsApp bot handler | `whatsapp/` | Chat-based onboarding, OTP, portfolio, deposit, and withdrawal flow | See `docs/WHATSAPP_BOT.md` and `whatsapp/README.md` |
| PostgreSQL / Supabase schema | `db/`, `supabase/` | Off-chain user, event, and operational state | See the schema directories before running migrations |
| Vault client package | `packages/vault-client/` | Generated TypeScript client bindings for the vault contract | Regenerated by the contract-spec workflow |
| Vault UI package | `packages/vault-ui/` | Shared UI library for vault-facing screens | See `docs/FRONTEND_PACKAGES.md` |
| Bridge package | `packages/bridge/` | Cross-chain bridge integration surface | Package workspace |
| Monitoring package | `packages/monitoring/` | Operations and alerting integration surface | Package workspace |
| Admin panel package | `packages/admin-panel/` | Operator-facing administrative UI surface | Package workspace |
...
> For component-specific setup, use the workspace links in [Workspace Layout](#workspace-layout).
